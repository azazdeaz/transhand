export default class Cursor {
  constructor(color) {
    ...
    this.FINGER2CURSOR = {
      'move': 'move',
      'rotate': getRotateCursor.bind(this),
      'origin': 'crosshair',
      '1000': 'ns-resize',
      '1100': 'nesw-resize',
      '0100': 'ew-resize',
  ...
  getCursor (props, state, x, y){
    ...
    if (typeof cursor === 'string' && cursor.indexOf('resize') !== -1) {
      cursor = getScaleCursor.bind(this)
    }
...
function getRotateCursor(props, state, mx, my) {
  ...
  var svg = escape(`<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" ><path transform="rotate(${r}, 16, 16)" d="M18.907 3.238l-7.54-2.104s8.35 3.9 8.428 15.367c.08 11.794-7.807 14.49-7.807 14.49l7.363-1.725" stroke="${this.color}" stroke-width="2.054" fill="none"/></svg>`)
...
function getScaleCursor(props, state) {
  ...
  var svg = escape(`<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"><path transform="rotate(${r}, 16, 16)" d="M22.406 12.552l5.88 4.18H3.677l5.728 4.36" stroke="${this.color}" stroke-width="2.254" fill="none"/></svg>`)
  return `url(data:image/svg+xml;utf8,${svg}) 16 16, all-scroll`
}
